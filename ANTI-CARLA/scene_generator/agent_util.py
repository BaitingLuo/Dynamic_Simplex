...
from samplers.Manual import Manual_Search
...
def write_sampler_results(route_path,folder,parameter_values,joined_parameters,data_path,route_run_folder):
    """
    ...
    with open(folder + "/changing_weather.csv", 'w') as csvfile2: #Always save the selected hyperparameters for optimization algorithms
        writer = csv.writer(csvfile2, delimiter = ',')
        writer.writerow(parameter_values)

    with open(route_run_folder + "/weather_across_run.csv", 'a') as csvfile1: #Always save the selected hyperparameters for optimization algorithms
        writer = csv.writer(csvfile1, delimiter = ',')
        writer.writerow(parameter_values)


def parameter_sampler(path,dynamic_parameters,static_parameters,folder,simulation_run,route_path,y,sampler,data_path,total_scenes,scene_num,initial_condition,constraints,route_run_folder,tracks_num):
    """
    ...
    if sampler == "Random":
        parameter_values, sampled_parameters = Random_Search(path,dynamic_parameters,folder,simulation_run,route_path,y,scene_num,initial_condition,weather_step,traffic_step,pedestrian_step,tracks_num)
    if sampler == "Grid":
        parameter_values, sampled_parameters = Grid_Search(path,dynamic_parameters,folder,simulation_run,route_path,y,initial_condition,constraints,weather_step,traffic_step,pedestrian_step,tracks_num)
    if sampler == "Manual":
        parameter_values, sampled_parameters = Manual_Search(path,dynamic_parameters,folder,simulation_run,route_path,y,scene_num,initial_condition,weather_step,traffic_step,pedestrian_step,tracks_num)
    joined_parameters = sampled_parameters + static_parameters

    write_sampler_results(route_path,folder,parameter_values,joined_parameters,data_path,route_run_folder)
...
def decode_scene_description(scene_config):
    ...
    for entry in scene_config['Scenario Description']:
        if entry == 'weather':
            ...
            track = scene_config['Scenario Description']['track']
    ...
    return varying_entities, varying_parameters, data_val, initial_condition, initial_location, initial_traffic_density, initial_pedestrian_density, constraints, track


def generate_scene_artifact_files(path,scene_config,sampler,folder,simulation_run,route_path,y,data_path,total_scenes,data_file,scene_num,route_run_folder,tracks_num):
    """
    All the processing scripts for scenes go here
    """
    weather_data = []
    joined_parameter_list = []
    varying_scene_entities, varying_scene_parameters, data_val, initial_condition, initial_location, initial_traffic_density, initial_pedestrian_density, constraints, track = decode_scene_description(scene_config)
    static_parameters, dynamic_parameters = prepare_data(data_val)
    joined_parameter_list = parameter_sampler(path,dynamic_parameters,static_parameters,folder,simulation_run,route_path,y,sampler,data_path,total_scenes,scene_num,initial_condition,constraints,route_run_folder,tracks_num)
    ...
    label_folder = path + "/leaderboard/data/labels/"
    carla_route = path + '/leaderboard/data/town%d_track%d_route/'%(town_number,track)
    print(carla_route)
    ...
    if simulation_run == 0 and scene_num == 1:
        carla_route  = carla_route + "track%s.xml"%initial_location
    else:
        carla_route = carla_route + "track%s.xml"%scene_num   #%track_list[scene_num-1]

    global_route,town = parse_routes_file(carla_route,False) #global route by reading one route from CARLA AD
    artifact_generator(carla_route,weather,scene_num,town_info,folder,route_run_folder,label_folder)
