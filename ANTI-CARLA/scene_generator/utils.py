...
def artifact_generator(route_filename,weather,id,town,folder,route_run_folder,label_folder):
    tree = ET.parse(route_filename)
    root = tree.getroot()
    ...
    label_file = label_folder + "track%s.csv"%my_id

    file = folder + "/label.csv"

    shutil.copy(label_file,file)
