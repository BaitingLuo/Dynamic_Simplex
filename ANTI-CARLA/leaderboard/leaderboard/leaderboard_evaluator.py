...
from team_code.utils import extract_collision_data
from team_code.utils import write_to_state_file
...
class LeaderboardEvaluator(object):
    ...
    def _load_and_run_scenario(self, args, config, sensors_info, data_to_record, record_frequency, display, record_folder,data_folder,route_folder,traffic_amount,tracks_num,resource_file):
        """
        ...
        try:
            ...
            self.agent_instance = getattr(self.module_agent, agent_class_name)(args.agent_config, sensors_info, data_to_record, record_frequency, display, record_folder, data_folder,args.project_path,route_folder,tracks_num,args.failure_type,args.configuration)
        ...
        try:
            ...
            if args.record:
                ...
                #self.client.start_recorder(data_folder + "/recorder.log".format(args.record, config.name, config.repetition_index))
                self.client.start_recorder(data_folder + "collision_recorder.log".format(args.record, config.name, config.repetition_index))
        ...
        # Run the scenario
        # try:
        self.manager.run_scenario(resource_file)

        try:
            print("\033[1m> Stopping the route\033[0m")
            self.manager.stop_scenario()
            print(self.client.show_recorder_collisions(data_folder + "collision_recorder.log", "v", "a"))
            file1 = open(data_folder + "collision_recorder.txt", "a")
            file1.writelines(self.client.show_recorder_collisions(data_folder + "collision_recorder.log", "v", "a"))
            # Closing file
            file1.close()
            self._register_statistics(config, args.checkpoint, entry_status, crash_message)
    ...
    def extract_collisions(self,data_folder):
        """
        Extract collision data for failure prediction
        """
        collision_file = data_folder + "collision_recorder.txt"
        state_data_file = data_folder + "state_data.csv"
        temp_file = data_folder + "temp.csv"
        collision_times = extract_collision_data(collision_file)
        #print(collision_times)
        #if len(collision_times) != 0:
        write_to_state_file(state_data_file,temp_file,list(set(collision_times)))
        # else:
        #     shutil.move(state_data_file,temp_file)

    def run(self, args, sensors_info, data_to_record, record_frequency, display, record_folder,data_folder,route_folder,data_root,run_root,infractions_to_record,tracks_num):
        """
        ...
        resource_file = data_folder + "resource_data.csv"
        ...
        while route_indexer.peek():
            ...
            # run
            self._load_and_run_scenario(args, config, sensors_info, data_to_record, record_frequency, display, record_folder,data_folder,route_folder,traffic_amount,tracks_num,resource_file)
        ...
        self.extract_collisions(data_folder)
...
def decode_agent_description(agent_config):
    ...
    record_frequency = agent_config['Record Frequency']
...
def read_folder_number(path,routes_folder):
    ...
    y = file1.read()
    file1.close() #to change file access modesimage_folder = paths[2] + "scene%d"%arguments.simulation_number + '/'
...
def main():
    ...
    parser.add_argument('--record', type=str, default='',
                        help='Use CARLA recording feature to create a recording')
    parser.add_argument('--timeout', default="360.0",
    ...
    parser.add_argument('--state_folder', type=str, help='Location to store recorded data by sensors')
    parser.add_argument('--sdl', type=str, help='Location of scenario description files')
    parser.add_argument('--tracks', type=int, help='Location of scenario description files')
    parser.add_argument('--failure_type', type=int, help='0 indicates no failure, 1 indicates permanent failure, 2 indicates intermittent failure')
    parser.add_argument('--configuration', type=str, help='DS: Dynamic Simplex; GS: Greedy Simplex; SA: Simplex Architecture; LBC:Performant Controller; AP: Safety Controller')
    ...
    try:
        ...
        tracks_num = arguments.tracks * arguments.simulation_number + arguments.scene_number
        #state_folder = paths[3] + "scene%d"%arguments.simulation_number + '/'
        #os.makedirs(state_folder, exist_ok=True)
        with open('scene_num.txt', 'w') as f:
            f.write(str(arguments.simulation_number+1))
        ...
        leaderboard_evaluator.run(arguments, sensors_info, data_to_record, record_frequency, display, record_folder,data_folder,route_folder,paths[0],run_root,infractions_to_record,tracks_num)
