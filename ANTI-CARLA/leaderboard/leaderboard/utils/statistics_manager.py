...
class StatisticsManager(object):
    ...
    def save_global_record(route_record,infractions_to_record, sensors, total_routes, endpoint,filename,stats_file):
        ...
        myvalues = [{'Driving score': '{:.3f}'.format(stats_dict['scores']['score_composed']),'Route completion': '{:.3f}'.format(stats_dict['scores']['score_route']),
                    'Infraction penalty': '{:.3f}'.format(stats_dict['scores']['score_penalty']), 'Collisions with pedestrians': '{:.3f}'.format(stats_dict['infractions']['collisions_pedestrian']),
                    'Collisions with vehicles': '{:.3f}'.format(stats_dict['infractions']['collisions_vehicle']), 'Collisions with layout': '{:.3f}'.format(stats_dict['infractions']['collisions_layout']),
                    'Red lights infractions': '{:.3f}'.format(stats_dict['infractions']['red_light']),
                    'Stop sign infractions': '{:.3f}'.format(stats_dict['infractions']['stop_infraction']),
                    'Off-road infractions': '{:.3f}'.format(stats_dict['infractions']['outside_route_lanes']),
                    'Route deviations': '{:.3f}'.format(stats_dict['infractions']['route_dev']),
                    'Route timeouts': '{:.3f}'.format(stats_dict['infractions']['route_timeout']),
                    'Agent blocked': '{:.3f}'.format(stats_dict['infractions']['vehicle_blocked'])}]
        ...
        with open(stats_file, 'a') as csvfile1:
            writer = csv.DictWriter(csvfile1, fieldnames = data['labels'])
            if not file_exists:
                writer.writeheader()
            writer.writerows(myvalues)
