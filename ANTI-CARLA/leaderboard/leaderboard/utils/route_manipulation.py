...
def scene_wp_to_gps(route, lat_ref, lon_ref):
    """
        Locate each waypoint of the route into gps, (lat long ) representations.
    :param route:
    :param lat_ref:
    ...
    for location in route:
        gps_point = _location_to_gps(lat_ref, lon_ref, location)
        gps_route.append(gps_point)
...
def interpolate_trajectory(world, waypoints_trajectory, hop_resolution=1.0):
    ...
    for i in range(len(waypoints_trajectory) - 1):   # Goes until the one before the last.
        ...
        for wp_tuple in interpolated_trace:
            ...
            wp_route.append(wp_tuple[0])
    lat_ref, lon_ref = _get_latlon_ref(world)

    return location_route_to_gps(route, lat_ref, lon_ref), route, wp_route, scene_wp_to_gps(waypoints_trajectory, lat_ref, lon_ref)
