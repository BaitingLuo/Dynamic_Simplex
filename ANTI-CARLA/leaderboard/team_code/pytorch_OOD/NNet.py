    ...
    def train(self, examples):
        ...
        for epoch in range(args.epochs):
            ...
            for _ in t:
                ...
                states, OOD = list(zip(*[training_data[i] for i in sample_ids]))
                states = torch.FloatTensor(np.array(states).astype(np.float))

                target_speed = torch.FloatTensor(np.array(OOD))
                # predict
                if args.cuda:
                    states, target_OOD = states.contiguous().cuda(), target_speed.contiguous().cuda()

                # compute output
                out_OOD = self.nnet(states)
                l_pi = self.loss_pi(target_OOD, out_OOD)
                total_loss = l_pi
                pi_losses.update(l_pi.item(), states.size(0))
                t.set_postfix(Loss_pi=total_loss)
            ...
            for _ in t_v:
                ...
                states, ood= list(zip(*[validation_data[i] for i in sample_ids]))

                states = torch.FloatTensor(np.array(states).astype(np.float))
                target_pis = torch.FloatTensor(np.array(ood))
                # predict
                if args.cuda:
                    states, target_ood = states.contiguous().cuda(), target_pis.contiguous().cuda()

                # compute output
                out_pi = self.nnet(states)
                val_pi = self.loss_pi(target_ood, out_pi)
                #total_loss = l_pi + l_v


                # record loss
                val_pi_losses.update(val_pi.item(), states.size(0))
                t_v.set_postfix(Loss_pi=val_pi_losses)
