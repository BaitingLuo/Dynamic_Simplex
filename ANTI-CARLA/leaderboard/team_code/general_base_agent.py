...
class BaseAgent(autonomous_agent.AutonomousAgent):
    def general_base_agent_setup(self, path_to_conf_file, parsed_sensors, data_to_record, record_frequency, display, record_folder, data_folder, project_path):
    ...
    def generate_failure(self,img, failure_type):
    ...
    def display_view(self,sensor_failure_happened):
        ...
        if len(self.view_data) == 0:
            ...
            #print("in base agent:",self.controller)
            #_draw_rgb = ImageDraw.Draw(rgb)
            rgb_tick_data_list = []
            for rgb_sensor in self.view_data.keys():
                #print(rgb_sensor)
                if sensor_failure_happened:
                    if rgb_sensor == 'rgb_camera_center':
                        rgb = self.generate_failure(self.view_data[rgb_sensor], 'occlusion')
                        self.view_data[rgb_sensor] = rgb
            ...
            if self.controller == "LBC":
                controller_name = "performant"
            else:
                controller_name = "safety"
            #font = ImageFont.truetype("/usr/share/fonts/truetype/freefont/FreeMono.ttf", size=15)
            font = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSansMono.ttf", size=15)
            _draw.text((5, 10), 'Controller:'+controller_name, font=font)
            _draw.text((5, 30), 'Speed: %.3f' % self.velocity, font=font)
